    function [V, converged, i] = fdpf(Ybus, Sbus, V0, Bp, Bpp, ref, pv, pq, mpopt, baseMVA, bus, branch)
...
%%--------------------
nb = npv + npq + 1;
idxgen = setdiff((1:nb)',pq);
coef = zeros(nb,1);
coef(idxgen) = 1 / length(idxgen);

%% evaluate initial mismatch
%%--------------------
if mpopt.isD
    loss = get_losses(baseMVA, bus, branch) ./ baseMVA;
    delta_P = real(sum(Sbus(Vm))-sum(loss));
    mis = (V .* conj(Ybus * V) - (Sbus(Vm) - delta_P * coef)) ./ Vm;
else
