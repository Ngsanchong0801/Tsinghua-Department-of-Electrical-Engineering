...
%%--------------------
for k = 1:length(pv)
    idxgen = find(gbus == pv(k));              %% which is(are) the reference gen(s)?
    Pd_genk = total_load(bus(pv(k), :), [], 'bus', [], mpopt);
    gen(on(idxgen(1)), PG) = real(Sbus(idxgen(1))) * baseMVA + Pd_genk; %% inj P + local Pd
    if length(idxgen) > 1       %% more than one generator at this ref bus
        %% subtract off what is generated by other gens at this bus
        gen(on(idxgen(1)), PG) = gen(on(idxgen(1)), PG) ...
                                - sum(gen(on(idxgen(2:length(idxgen))), PG));
    end
end
