...
matrix_n = time;
delta_PG_tm = pi*(delta_f_cr^2)*KG_gen.*S_gen*Tj_sys/(4*TG_sys*delta_PL*S_sys);
...
gen_state_value = value(gen_state);
save('1_FCUC_BJ_gen_state','gen_state_value');
gen_generate_value = value(gen_generate);
save('1_FCUC_BJ_gen_generate','gen_generate_value');
Tj_value = value(Tj_sys);
D_value = value(D_sys);
KG_value = value(KG_sys);
save('1_FCUC_BJ_Tj','Tj_value');
save('1_FCUC_BJ_D','D_value');
save('1_FCUC_BJ_KG','KG_value');