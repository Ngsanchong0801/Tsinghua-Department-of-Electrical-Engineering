...
% 频率最低点约束中的发电机实际调频功率
delta_PG = sdpvar(length(gen(:,1)),time,'full');
...
% 系统参数（96列行向量）
FR_sys = FR_gen'*(gen_state.*repmat(S_gen,1,time))/S_sys;
% 最大频率变化率RoCoFmax（标幺值）
...
% 频率最低点约束中的delta_PG_tm（矩阵）
matrix_m = length(gen(:,1));
matrix_n = time;
delta_PG_tm = repmat(FR_gen.*KG_gen.*(-delta_f_cr),1,time) + KG_gen.*(1-FR_gen)*Tj_sys*((-delta_f_cr)^2)*pi/(4*TR_gen*delta_PL);
...
% 频率最低点约束
% cons = [cons,sum(delta_PG,1) - delta_f_cr*D_sys >= delta_PL];
% cons = [cons,delta_PG <= delta_PG_tm];
% cons = [cons,delta_PG <= (gen_state.*repmat(S_gen,1,time)-gen_generate)/S_sys];
...
for i =1:4
    cons = [cons,a(1,i)*KG_sys + b(1,i)*FR_sys + d(1,i)*Tj_sys + d(1,i)<= delta_f_cr];
end
% 最大频率变化率约束/准稳态频率偏差约束
...
gen_state_value = value(gen_state);
save('1_FCUC_F_gen_state','gen_state_value');
gen_generate_value = value(gen_generate);
save('1_FCUC_F_gen_generate','gen_generate_value');
Tj_value = value(Tj_sys);
D_value = value(D_sys);
KG_value = value(KG_sys);
save('1_FCUC_F_Tj','Tj_value');
save('1_FCUC_F_D','D_value');
save('1_FCUC_F_KG','KG_value');