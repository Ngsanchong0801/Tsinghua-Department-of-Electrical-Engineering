...
for sam = 1:N
    ...
    cons = [cons,bus_P(bus_index_hasGen,:) == bus_if_hasWind(bus_index_hasGen,1)*gen_wind(1,:) + gen_generate(gen_index,:) - load(bus_index_hasGen,:)];%发电机节点
    ...
    % 风机出力约束
    cons = [cons,gen_wind(:,:) >= 0];
    cons = [cons,gen_wind(:,:) <= Pwmax(:,sam)'];
    ...
    % 增加到目标函数中
    object = object + (0.25*sum(gen_cost_wind'*gen_wind(:,:)) ...
        + 0.25*sum(gen_cost_1'*gen_generate(:,:)) + 0.25*sum(gen_cost_0'*gen_state(:,:)) + sum(sum(gen_cost_updown(:,:))))/N;
end
...
figure(2);
hold on;
stairs(time_vector,gen_wind_value);
hold off;
