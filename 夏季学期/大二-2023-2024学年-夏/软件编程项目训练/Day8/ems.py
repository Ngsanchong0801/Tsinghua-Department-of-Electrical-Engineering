...
from emsui import Ui_MainWindow


class EmsApp(QMainWindow, Ui_MainWindow):
    def __init__(self, rtu_serial_number):
        super().__init__()
        self.setupUi(self)
        self.labelRtuId.setText(rtu_serial_number)


def main():
    app = QApplication(sys.argv)

    rtu_serial_number = sys.argv[1] if len(sys.argv) > 1 else ""

    wnd = EmsApp(rtu_serial_number)
    wnd.show()
