...
%系统功率平衡约束
cons=[cons,load_winter(1,:)'==hydro(:,1).*(hydro_all+sum(hydro_backup.*hydro_back_work))+fire(:,1:15).*fire_if_work(:,1:15)*fire_all+wind(:,1).*(wind_all+sum(wind_backup.*wind_back_work'))+solar(:,1).*(solar_all+sum(solar_backup.*solar_back_work'))+nuclear(:,1).*nuclear_all*if_nuclear];
cons=[cons,load_summer(1,:)'==hydro(:,2).*(hydro_all+sum(hydro_backup.*hydro_back_work))+fire(:,16:30).*fire_if_work(:,16:30)*fire_all+wind(:,2).*(wind_all+sum(wind_backup.*wind_back_work'))+solar(:,2).*(solar_all+sum(solar_backup.*solar_back_work'))+nuclear(:,2).*nuclear_all*if_nuclear];
...
cons=[cons,0<=wind_back_work<=60];
cons=[cons,0<=solar_back_work<=60];
...
Y1=[hydro_power1,fire_power1,wind_power1,solar_power1,nuclear_power1];
Y2=[hydro_power2,fire_power2,wind_power2,solar_power2,nuclear_power2];
x=1:96;
...
a1=area(ax1,x,Y1);
axis([1 96 0 3000]);
title('冬季总出力曲线'),xlabel('时间/点'),ylabel('总出力/万千瓦'),legend({'水电','火电','风电','光伏','核电'})

ax2 = nexttile;
a2=area(ax2,x,Y2);
axis([1 96 0 3000]);
title('夏季总出力曲线'),xlabel('时间/点'),ylabel('总出力/万千瓦'),legend({'水电','火电','风电','光伏','核电'})
