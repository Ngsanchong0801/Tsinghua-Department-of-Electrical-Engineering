...
%系统功率平衡约束
cons=[cons,load_winter(1,:)'*times==hydro(:,1).*(hydro_all+sum(hydro_backup.*hydro_back_work))+fire(:,1:15).*fire_if_work(:,1:15)*fire_all+wind(:,1).*(wind_all+sum(wind_backup.*wind_back_work'))+solar(:,1).*(solar_all+sum(solar_backup.*solar_back_work'))+nuclear(:,1).*nuclear_all*if_nuclear];
cons=[cons,load_summer(1,:)'*times==hydro(:,2).*(hydro_all+sum(hydro_backup.*hydro_back_work))+fire(:,16:30).*fire_if_work(:,16:30)*fire_all+wind(:,2).*(wind_all+sum(wind_backup.*wind_back_work'))+solar(:,2).*(solar_all+sum(solar_backup.*solar_back_work'))+nuclear(:,2).*nuclear_all*if_nuclear];
