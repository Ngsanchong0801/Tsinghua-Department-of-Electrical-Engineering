% 读取数据
data = readtable('Signal_extra1_15.csv', 'VariableNamingRule', 'preserve');
V_val = data{:,2};  % 直接提取第二列数据
...
% 小波包分解和去噪
wp = wpdec(V_val, level, wname);
[thr, sorh, keepapp] = ddencmp('den', 'wp', V_val);
V_val_denoised = wdenoise(V_val, level, 'Wavelet', wname);

% 使用卡尔曼滤波器计算基波相量
[result1, result2, result3, result4] = karman1(V_val_denoised, 0.0001);
...
% 计算信噪比（SNR）
signal_power = bandpower(V_val_denoised, 10000, [0 5000]);
noise_power = bandpower(V_val - V_val_denoised, 10000, [0 5000]);
SNR = 10 * log10(signal_power / noise_power);

disp(['信噪比 (SNR): ', num2str(SNR), ' dB']);

% Zigzag 拟合
x_int = linspace(0, 9999, length(V_val_denoised));
sig = V_val_denoised;
f2 = @(c,x)( (x>=0 & x<c(1)).*0 + ...
             (x>=c(1) & x<c(2)).*c(3)./(c(2)-c(1)).*(x-c(1)) + ...
             (x>=c(2) & x<c(4)).* ((c(5)-c(3))./(c(4)-c(2)).*(x-c(2))+c(3)) + ...
             (x>=c(4) & x<c(6)).*c(5)./(c(4)-c(6)).*(x-c(6)) + ...
             (x>=c(6)).*0 );
c0 = [1100, 1500, 1800, 5400, 2500, 5600];  % 初始猜测
